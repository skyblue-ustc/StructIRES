...
def main() -> int:
    ...
    colors = ["#7f8c8d", "#d95f02", "#1b9e77", "#386cb0"]
    panels = [
        ("lm_log_likelihood_per_token", "RNA-LM likelihood\n(higher is better)"),
        ("mfe_abs_delta_kcal_mol", r"$|\Delta$MFE| (kcal/mol)\n(lower is better)"),
        ("pairing_profile_l1", r"Pairing-profile $L_1$\n(lower is better)"),
    ]
    ...
    for axis, (metric, title) in zip(axes, panels):
        ...
        axis.set_xticks(range(4), [LABELS[key] for key in ORDER], rotation=28, ha="right")
