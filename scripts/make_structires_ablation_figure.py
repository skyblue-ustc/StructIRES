#!/usr/bin/env python3
"""Render the matched released-RNA-FM constraint ablation figure."""
...
LABELS = {
    "rnafm_score_only": "RNA-FM\nscore only",
    "rnafm_plus_energy": "+ energy",
    "rnafm_plus_ensemble": "+ ensemble",
...
def main() -> int:
    ...
    colors = ["#9e9e9e", "#e69f00", "#56b4e9", "#0072b2"]
    panels = [
    ...
    for axis, (metric, title, note) in zip(axes[:2], panels):
        ...
        axis.bar(range(4), values, yerr=errors, capsize=3, color=colors, edgecolor="black", linewidth=0.5)
    ...
    axes[2].bar(range(4), values, color=colors, edgecolor="black", linewidth=0.5)
    axes[2].set_title("Held-out direct-RNA proxy", fontsize=10)
    axes[2].text(0.02, 0.97, "higher is better", transform=axes[2].transAxes, va="top", fontsize=8)
    axes[2].grid(axis="y", alpha=0.25)
    for axis in axes:
        axis.set_xticks(range(4), [LABELS[key] for key in ORDER], fontsize=8)
