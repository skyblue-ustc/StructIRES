...
### 5. The released random split is repeated holdout, not ordinary ten-fold CV

The released long-form split contains ten independently sampled 90/10 stratified holdouts. Across
46,774 unique sequences, 16,176 never occur in a released test set, while other sequences occur in
two to six released test sets. It therefore cannot be decoded as one mutually exclusive test-fold
assignment per sequence. In addition, the released RNA-FM and UTR-LM training scripts select the
best epoch using test-fold AUPR (`IRES_RNAFM.py:474` and `IRES_UTRLM.py:434`). For this project:

- published numbers remain clearly labelled as external references;
- exact upstream-protocol reproduction, if needed, is reported as a diagnostic only;
- project random-split pilots use deterministic mutually exclusive folds and a separate validation
  fold for thresholds/checkpoints;
- headline claims require frozen sequence-cluster/family splits.

The executable audit and exact assignment-count distribution are documented in
`docs/DATA_SPLIT_AUDIT_2026-08-21.md`.
