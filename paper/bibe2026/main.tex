...
\usepackage{xcolor}
\input{tables/recognition_native_rows}
...
structure-aware classifier starts from public RNA-FM initialization, pools
contextual residue pairs over label-free ViennaRNA MFE contacts, and injects a
gated residual into an otherwise matched IRES head. Across
\StructIRESNativeFoldCount validation-clean native folds, contact fusion changes AUROC/AUPR/F1 from
$\StructIRESNativeSeqAUC$/$\StructIRESNativeSeqAUPR$/$\StructIRESNativeSeqFOne$ to
$\StructIRESNativeContactAUC$/$\StructIRESNativeContactAUPR$/$\StructIRESNativeContactFOne$ relative to sequence-only
retraining. This is a controlled legacy-reporter benchmark result; a stricter
identity-aware profile-fusion audit does not improve ranking. Its
\textsc{StructIRES-Rank} component is a generator-agnostic, assay-aware
rank-selection layer that combines a released IRES language-model ensemble
with parent-relative energy, global pairing-profile, and high-confidence
ensemble-anchor constraints. In a controlled experiment over
...
verified translation improvement. Recognition findings are not inferred from
design outcomes.
\end{abstract}
