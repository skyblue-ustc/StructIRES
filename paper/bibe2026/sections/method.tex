...
\textsc{StructIRES-Classifier} is defined as a validation-trained fusion of
RNA foundation-model sequence scores with thermodynamic ensemble descriptors:
minimum free energy (with length-normalized variants), ensemble diversity, and
summary statistics of the base-pairing-probability profile. The planned head,
feature preprocessing, regularization strength, and decision threshold are
selected using training/validation records only; the official test partition is
not used for model selection. This component is included as a locked
next-experiment specification in the present draft. Its performance cells are
therefore explicitly marked ``TBD'' until predictions, manifests, and a
reproducible test report exist.
...
completed runs is the released IRES-LM ensemble; a future locked rerun may
substitute the \textsc{StructIRES-Classifier} score only after its
identification evaluation is complete. These are predicted thermodynamic
features, not experimentally mapped IRES motifs. The combined method ranks the
